const RUNNER_PREFIX = "phptestsuite.runner.prefix";
...
function getWorkingDirPath() {
  let workingDir =
    nova.workspace.config.get("tests", "string") || nova.workspace.path;
  if (!workingDir) {
    return null;
  }
...
function getTestRunner() {
  if (!nova.fs.stat(nova.path.join(nova.workspace.path, "composer.json"))) {
    nova.workspace.showErrorMessage(
      "Project must contain composer.json file in order to run tests.",
    );
    return;
  }

  const fileContent = nova.fs.open(
    nova.path.join(nova.workspace.path, "composer.json"),
  );
...
function getTestMethod(text) {
  ...
  const matchUnit = text
    .trim()
    .match(/^\s*(?:public|private|protected)?\s*function\s*(\w+)\s*\(.*$/);
...
class TestDataProvider {
  constructor() {
    ...
    additionalPaths.forEach((folder) => {
      if (nova.fs.stat(nova.path.join(getWorkingDirPath(), folder))) {
        const packages = nova.fs.listdir(
          nova.path.join(getWorkingDirPath(), folder),
        );

        packages.forEach((item) => {
          this.findTests(
            nova.path.join(getWorkingDirPath(), `${folder}/${item}`),
          );
        });
      }
    });
  ...
  getTreeItem(element) {
    ...
    if (element.children.length > 0) {
      item.collapsibleState = nova.workspace.config.get(
        `${element.name}-expanded`,
      )
        ? TreeItemCollapsibleState.Expanded
        : TreeItemCollapsibleState.Collapsed;
      item.image = isPest
        ? "../Images/sidebar-pest-icon/sidebar-pest-icon.png"
        : "../Images/sidebar-phpunit-icon/sidebar-phpunit-icon.png";
